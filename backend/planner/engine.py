...
CITY_DB = {
    ...
    # You can add the rest of the cities from the previous constants.ts here
}
...
def calculate_trip(data):
    ...
    stops = [{"location": {"name": data['currentLocation'], "coords": current_loc}, "type": "start"}]
    ...
    for target_city, type_lbl in legs:
        ...
        while dist > 0:
            ...
            if time_needed > drive_avail:
                ...
                # Take Break
                add_event("OFF", 0.5, "Rest Area", "Mandatory 30m Break")
                stops.append({"location": {"name": "Rest Area", "coords": curr_pos}, "type": "rest"})
        ...
        # Arrived
        add_event("ON", 1.0, target_city, f"Loading/Unloading {type_lbl}")
        stops.append({"location": {"name": target_city, "coords": target_coords}, "type": type_lbl})
